    ...
    public class VectorStringConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            return value is Point3D ? ToDisplay((Point3D)value)
                : value is Vector3D ? ToDisplay((Point3D)(Vector3D)value)
        ...
        {
            return string.Format("({0:N3}, {1:N3}, {2:N3})", p.X, p.Y, p.Z);
        }
        ...
        {
            return string.Format("Axis: {0}, Angle: {1:N1} °", ToDisplay((Point3D)q.Axis), q.Angle);
        }
    }
}
